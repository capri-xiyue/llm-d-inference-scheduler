...
func getKVCacheBlocksFromChatCompletions(ctx context.Context, request *scheduling.InferenceRequest, blockSizeTokens int, tokenEstimator TokenEstimator) []KVCacheBlock {
	...
	for _, msg := range messages {
		if msg.Role != "" {
			appendTextTokens(msg.Role, &allPseudoTokens)
		}
		if msg.Content.Raw != "" {
			appendTextTokens(msg.Content.Raw, &allPseudoTokens)
		} else if len(msg.Content.Structured) > 0 {
			for _, block := range msg.Content.Structured {
				switch block.Type {
				case "text":
					appendTextTokens(block.Text, &allPseudoTokens)
				case "image_url":
					...
					for i := 0; i < numPlaceHolders; i++ {
						allPseudoTokens = append(allPseudoTokens, imgHash)
					}
				case "video_url":
					// Add video support later
					appendTextTokens(block.VideoURL.URL, &allPseudoTokens)
				case "input_audio", "audio_url":
					// Add audio support later
					appendTextTokens(block.InputAudio.Data, &allPseudoTokens)
					appendTextTokens(block.InputAudio.Format, &allPseudoTokens)
				default:
...
func appendTextTokens(text string, allTokens *[]string) {
